    ...
    float jitterY)
{
    ...
    if (edge > edgeSensitivity && colourBleed > 0.0f)
    {
        ...
        if (weight > 0.0f)
        {
            out.x = out.x*(1-colourBleed) + (sumR/weight)*colourBleed;
            out.y = out.y*(1-colourBleed) + (sumG/weight)*colourBleed;
            out.z = out.z*(1-colourBleed) + (sumB/weight)*colourBleed;
            out.w = out.w*(1-colourBleed) + (sumA/weight)*colourBleed;
        }
    }
    ...
    if (crushBlacks > 0.0f)
    {
        float lum = 0.299f*out.x + 0.587f*out.y + 0.114f*out.z;
        float shadowWeight = (1.0f - lum) * (1.0f - lum);
        float crush = fmaxf(0.0f, 1.0f - crushBlacks * shadowWeight);
        out.x *= crush;
        out.y *= crush;
        out.z *= crush;
    }
    ...
    {
        float lum    = 0.299f*out.x + 0.587f*out.y + 0.114f*out.z;
        float chroma = fmaxf(fmaxf(out.x, out.y), out.z)
                     - fminf(fminf(out.x, out.y), out.z);
        float boost  = 1.0f - chroma * vibranceFocus;
        float sat    = 1.0f + vibrance * boost;
        out.x = fmaxf(0.0f, fminf(lum + (out.x - lum) * sat, 1.0f));
        out.y = fmaxf(0.0f, fminf(lum + (out.y - lum) * sat, 1.0f));
        out.z = fmaxf(0.0f, fminf(lum + (out.z - lum) * sat, 1.0f));
    }
    ...
    A_u_long               /*device_index*/)
{
    ...
    CelluloseKernel<<<gridDim, blockDim>>>(
        ...
        cp.crushBlacks, cp.vibrance, cp.vibranceFocus, cp.currentTime, jitterX, jitterY);
